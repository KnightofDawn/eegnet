            ...
            with slim.arg_scope([slim.conv2d, slim.fully_connected], 
                ...
                with tf.variable_scope('hidden'):
                    ...
                    with tf.variable_scope('layer5'):
                        layer_input = hidden
                        # Compress layer input features
                        hidden = slim.conv2d(hidden, 32, 1, scope='1x1compress')
                        # Dilated convolution. NO nonlinear and batch norm!
                        hidden = slim.conv2d(hidden, 8, [1, 3], rate=2, normalizer_fn=None, 
                                             activation_fn=None, scope='dilconv')
                        # Split features in half
                ...
                with tf.variable_scope('skip_processing'):
                    ...
                    hidden = slim.avg_pool2d(hidden, [1, batch_num_points*2//200], [1, batch_num_points//200])
                    # 1 x 2400 x 32
                    hidden = slim.conv2d(hidden, 16, 1, scope='1x1compress')
                    hidden = slim.conv2d(hidden, 8, [1, 3], stride=3, scope='1x3reduce')
                    # 1 x 800 x 8
                    hidden = slim.conv2d(hidden, 2, [1, 3], stride=4, scope='1x3reduce')
                    # 1 x 200 x 2
